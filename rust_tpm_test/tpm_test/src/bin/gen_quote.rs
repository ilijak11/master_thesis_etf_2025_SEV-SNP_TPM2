...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (attest, signature, ak_pub) = get_quote(nonce.to_be_bytes().to_vec())?;
...
fn get_quote(nonce: Vec<u8>) -> Result<(Attest, Signature, Public), Box<dyn std::error::Error>> {
